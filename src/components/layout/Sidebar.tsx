...
export const Sidebar = forwardRef<HTMLElement, SidebarProps>(({ isOpen, currentUser, navigationLinks, currentPageKey, handleNavigate, handleAction, handleLogout }, ref) => {
    return (
        <aside ref={ref} className={`sidebar w-64 bg-gray-900 text-gray-100 p-4 space-y-2 flex flex-col fixed inset-y-0 left-0 z-30 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 dark:bg-gray-900 dark:border-r dark:border-gray-800`}>
