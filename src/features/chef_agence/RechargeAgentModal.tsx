...
export const RechargeAgentModal: React.FC<RechargeAgentModalProps> = ({ isOpen, onClose, onConfirm, agent, chef }) => {
    ...
    const chefBalanceAfter = typeof amount === 'number' ? chef.solde - amount : chef.solde;
    const agentBalanceAfter = typeof amount === 'number' ? agent.solde + amount : agent.solde;

    return (
        <Modal id="recharge-agent-modal" title={`Recharger le solde de ${agent.name}`} isOpen={isOpen} onClose={onClose}>
