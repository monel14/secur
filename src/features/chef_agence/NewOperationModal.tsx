...
import { OperationType, ChefAgence, FormField, CommissionConfig } from '../../types';
...
export const NewOperationModal: React.FC<NewOperationModalProps> = ({ isOpen, onClose, user, onSave }) => {
    ...
    useEffect(() => {
        ...
        const requiredFields = selectedOpType.fields.filter((f: any) => f.required);
        const allRequiredFieldsFilled = requiredFields.every((field: any) => formData[field.name] && formData[field.name] !== '');
    ...
    const amountInput = opFields.find((f: any) => f.name.includes('montant'));
    ...
    return (
        <Modal id="chef-new-operation-modal" title="Initier une Opération" isOpen={isOpen} onClose={onClose} size="modal-lg">
            <form onSubmit={handleSubmit}>
                ...
                {selectedOpType && (
                    <>
                        <div className="p-4 border rounded-md bg-gray-50 min-h-[150px]">
                            ...
                            {opFields.map((field) => (
                                <div key={field.id}>
                                    ...
                                    {field.type === 'select' ? (
                                        <select id={field.id} name={field.name} className="form-select form-select-sm" value={formData[field.name] || ''} onChange={handleFieldChange} required={field.required}>
                        ...
                        <div className="my-4">
                            <label className="form-label" htmlFor="opProofChef">
