...
import { OperationType, FormField, CommissionTier, CommissionConfig } from '../../types';
...
export const DevEditOperationTypeModal: React.FC<DevEditOperationTypeModalProps> = ({ isOpen, onClose, opTypeId, initialData, onSave }) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            if (isOpen) {
                setActiveTab('info');
                let dataToLoad: OperationType | null = null;
                if (initialData) {
                    ...
                    if (error) console.error(error);
                        ...
                        dataToLoad = {
                            ...
                            commission_config: dbData.commission_config || { type: 'none' },
                        };
                    }
                } else {
                ...
                if (dataToLoad) {
                    const cleanData = JSON.parse(JSON.stringify(dataToLoad));
                    cleanData.fields = Array.isArray(cleanData.fields) ? cleanData.fields : [];
                    cleanData.commission_config = cleanData.commission_config && typeof cleanData.commission_config === 'object' ? cleanData.commission_config : { type: 'none' };
                    if (cleanData.commission_config.type === 'tiers' && !Array.isArray(cleanData.commission_config.tiers)) {
                        cleanData.commission_config.tiers = [];
                    }
                    setOpData(cleanData);
                } else {
                    setOpData(null);
    ...
    return (
        <Modal id="dev-op-type-modal" title={opTypeId ? `Éditer: ${opData.name}` : "Créer un Type d'Opération"} isOpen={isOpen} onClose={onClose} size="modal-xl">
            <form onSubmit={handleSave}>
                ...
                <div className="flex justify-end space-x-3 mt-6 pt-4 border-t">
                    ...
                    <button type="submit" className="btn btn-success"><i className="fas fa-save mr-2"></i>Enregistrer</button>
                </div>
            </form>
        </Modal>
    );
